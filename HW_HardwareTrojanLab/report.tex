...
\begin{center}
  ...
  \begin{tabular}{@{}l l@{}}
     {Alex Cote} & RIN: {662040076} \\
     {Gray Lampkin} & RIN: {662092126} \\
     {Thomas Jenkins} & RIN: {662049450}\\
  \end{tabular}\\[4pt]
...
\midrule
{Alex Cote} & {662040076} & Configured Python venv and API keys; fixed GHOST Unicode encoding errors; ran single-insertion experiment; wrote clean baseline Verilog designs \\
{Gray Lampkin} & {662092126} & Wrote batch generation script (\texttt{task2\_batch\_generation}); implemented regex fallback for malformed API responses; analyzed output metrics; produced \texttt{summary.csv} \\
{Thomas Jenkins} & {662049450} & Implemented \texttt{task4\_validation()} structural checks; built and evaluated \texttt{detector.py} heuristic engine; compiled \texttt{validation\_log.txt} \\
\bottomrule
\end{tabular}
\end{table}
