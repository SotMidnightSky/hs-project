...
{Thomas Jenkins} & {662049450} & Wrote batch generation script (\texttt{task2\_batch\_generation}); implemented regex fallback for malformed API responses; analyzed output metrics; produced \texttt{summary.csv} \\
{Gray Lampkin} & {[RIN]} & Implemented \texttt{task4\_validation()} structural checks; built and evaluated \texttt{detector.py} heuristic engine; compiled \texttt{validation\_log.txt} \\
\bottomrule
\end{tabular}
\end{table}
